--[[
The follwoing file is an edited version of "task-list.lua", downloaded from

    https://github.com/pandoc/lua-filters/blob/6bd1657/task-list/task-list.lua

| MIT License
| Copyright (c) 2017-2018 pandoc

Edited by Tristano Ajmone --- The following line (towards the end) was commented
out to prevent CSS injection into the document header, because it conflicted
with the "header-includes" defined in the YAML headers:
...
M[1] = {
  ...
  -- COMMENTED OUT TO PREVENT CSS INJECTION:
  -- Meta = is_html(FORMAT) and M.add_task_list_css or nil
}
